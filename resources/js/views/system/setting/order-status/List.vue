...
export default {
  ...
  methods: {
    create(){
      this.loading = true;
      orderStatusResource.store(this.dataTemp).then((res) => {
        if (res.success) {
    ...
    update(){
      this.loading = true;
      orderStatusResource.update(this.dataTemp.id,this.dataTemp).then((res) => {
        if (res.success) {
    ...
    handleDeleting(row){
      this.$confirm('This will permanently delete the row. Continue?', 'Warning', {
        ...
        orderStatusResource.destroy(row.id).then((res) => {
          if (res) {
