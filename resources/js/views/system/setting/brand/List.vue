    ...
    <el-row class="el-main-form" :gutter="20" style="margin:0px;">
      ...
      <el-col :span="16">   
        ...
        <el-table
          ...
          <el-table-column :label="$t('table.sort')" min-width="80px" align="center">
            <template slot-scope="scope">
              {{ scope.row.sort }}
            </template>
          </el-table-column>
...
export default {
  ...
  methods: {
    create(){
      this.loading = true;
      brandResource.store(this.dataTemp).then((res) => {
        if (res.success) {
    ...
    update(){
      this.loading = true;
      brandResource.update(this.dataTemp.id,this.dataTemp).then((res) => {
        if (res.success) {
    ...
    handleVisibleStorage(){
      this.$store.commit('fm/setDisks', 'brand');
