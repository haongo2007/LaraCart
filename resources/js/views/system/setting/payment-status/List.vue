...
import PaymentStatusResource from '@/api/payment-status';

const paymentStatusResource = new PaymentStatusResource();
...
export default {
  ...
  methods: {
    create(){
      this.loading = true;
      paymentStatusResource.store(this.dataTemp).then((res) => {
        if (res.success) {
    ...
    update(){
      this.loading = true;
      paymentStatusResource.update(this.dataTemp.id,this.dataTemp).then((res) => {
        if (res.success) {
    ...
    async getList() {
      const data = await paymentStatusResource.list();
      this.list = data.data;
    ...
    handleDeleting(row){
      this.$confirm('This will permanently delete the row. Continue?', 'Warning', {
        ...
        paymentStatusResource.destroy(row.id).then((res) => {
          if (res) {
