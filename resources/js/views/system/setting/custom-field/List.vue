    ...
    <el-row class="el-main-form" :gutter="20" style="margin:0px;">
      ...
      <el-col :span="16">
        <el-table
          ...
          <el-table-column :label="$t('table.code')" min-width="100px">
            <template slot-scope="scope">
              {{ scope.row.code }}
            </template>
          </el-table-column>
          ...
          <el-table-column :label="$t('table.required')" min-width="100px">
            <template slot-scope="scope">
              {{ scope.row.required ? 'true' : 'false' }}
            </template>
          </el-table-column>

          <el-table-column :label="$t('table.status')" min-width="80px">
          ...
          <el-table-column :label="$t('table.default')" min-width="100px">
            <template slot-scope="scope">
              {{ scope.row.default }}
            </template>
          </el-table-column>
...
import CustomFieldResource from '@/api/custom-field';

const customFieldResource = new CustomFieldResource();
...
export default {
  ...
  methods: {
    async getList() {
      const data = await customFieldResource.list();
      this.list = data.data;
