<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column :label="$t('table.top')" class-name="status-col" min-width="100px" prop="active">
        <template slot-scope="{row}">
          <el-switch
            v-model="row.top"
            :active-value="1"
            ...
            @change="handleChange(row.id,row.top,'top')"
          />
        </template>
      </el-table-column>
        ...
        <template slot-scope="{row}">
          <el-button-group>
            <el-button
              ...
              icon="el-icon-edit"
              class="filter-item"
              @click="renderRouterEdit(row.kind,row.id)"
            />
            <el-button
...
export default {
  ...
  methods: {
    ...
    handleChange(id, val, key){
      ...
      params[key] = val;
      EventBus.$emit('handleUpdateTop', params);
    },
  },
};
</script>
