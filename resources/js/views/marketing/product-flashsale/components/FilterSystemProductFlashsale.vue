        ...
        <el-row :gutter="20">
          <el-col :span="24">
            <el-button-group>
              ...
              @click="$router.push({ name: 'UserCreate'}).catch(() => {})" v-permission="['create.flashsale']" />
              <el-button type="danger" icon="el-icon-delete" :disabled="multiSelectRow.length == 0 ? true : false" 
              @click="handerDeleteAll" v-permission="['delete.flashsale']"/>
            </el-button-group>
          </el-col>
        </el-row>
      </div>
...
export default {
  name: 'FilterSystemProductFlashsale',
  ...
  methods: {
    async getList() {
      const data = await productFlashsaleResource.list(this.dataQuery);
      this.list = data.data;
    ...
    handleDeleting(row, multiple = false) {
      this.$confirm('This will permanently delete the row. Continue?', 'Warning', {
        ...
        var that = this;
        productFlashsaleResource.destroy(id).then((res) => {
          if (res) {
            if (multiple) {
