...
if (!function_exists('lc_check_view')) {
    /**
     * Check view exist
     *
     ...
     */
    function lc_check_view($view)
    {
        if(!view()->exists($view)) {
            lc_report('View not found '.$view);
...
if (!function_exists('lc_clean')) {
    /**
     * Clear data
     */
    function lc_clean($data = null, $exclude = [], $level_hight = null)
    {
        if ($level_hight) {
            if (is_array($data)) {
        ...
        if (is_array($data)) {
            array_walk($data, function (&$v, $k) use ($exclude, $level_hight) {
                if (is_array($v)) {
                    $v = lc_clean($v, $exclude, $level_hight);
                } else {
