...
<div class="col-md-12">
  <div class="card" >
    ...
    <div class="card-body" >
      <form action="{{bc_route_admin('admin_cms_category.index')}}" id="button_search">
        <div class="row justify-content-between">
            ...
            <div class="col-sm-7 d-flex justify-content-end">
                <div class="form-group d-flex">
                  <input type="text" name="keyword" class="form-control mr-3" value="{{$keyword}}" placeholder="{{ trans($pathPlugin.'::Category.admin.search_place') }}">
        ...
        <div class="tools float-right">
            <div class="dropdown">
                ...
                <div class="dropdown-menu dropdown-menu-right" x-placement="bottom-end">
                    <a class="dropdown-item" id="button_create_new" href="{{bc_route_admin('admin_cms_category.create')}}">{{trans('admin.add_new')}}</a>
          ...
          <table class="table box-body">
            <thead>
              <tr>
                ...
                @endif
                  <th>#</th>
                  <th>{{trans($pathPlugin.'::Category.image')}}</th>
                  <th>{{trans($pathPlugin.'::Category.title')}}</th>
                  <th class="text-center">{{trans($pathPlugin.'::Category.parent')}}</th>
                  <th class="text-center">{{trans($pathPlugin.'::Category.status')}}</th>
                  <th class="text-center">{{trans($pathPlugin.'::Category.sort')}}</th>
                  <th>{{trans($pathPlugin.'::Category.admin.action')}}</th>
               </tr>
            </thead>
            <tbody>
              @foreach ($dataCmsCate as $keyRow => $row)
              <tr>
                  @if (!empty($removeList))
                  ...
                  <td>{{ $row['title'] }}</td>
                  <td class="text-center">{{ $row['parent'] ? ($categoriesTitle[$row['parent']] ?? '') : 'ROOT'}}</td>
                  ...
                  <td>
                    @include($templatePathAdmin.'Component.action_list',['url_edit'=> bc_route_admin('admin_cms_category.edit',['id'=>$row['id']]),'id'=>$row['id']])
                  </td>
              </tr>
              @endforeach
            </tbody>
