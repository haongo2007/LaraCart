...
{
    public $table = BC_DB_PREFIX . 'admin_menu';
    protected $guarded = [];
    ...
    public function getTree($parent = 0, &$tree = null, $menus = null, &$st = '')
    {
        ...
        foreach ($lisMenu as $menu) {
            $tree[$menu->id] = $st . ' ' . bc_language_render($menu->title);
 ...
 */
    ...
    {
        try {
            DB::connection(BC_CONNECTION)->beginTransaction();
            foreach ($data as $key => $menu) {
                $this->where('id', $key)->update($menu);
            }
            DB::connection(BC_CONNECTION)->commit();
            $return = ['error' => 0, 'msg' => ""];
        } catch (\Throwable $e) {
            DB::connection(BC_CONNECTION)->rollBack();
