...
class User extends Model implements AuthenticatableContract
{
    ...
    {
        return $this->belongsToMany(Permission::class, 'admin_user_permission', 'user_id', 'permission_id');
    }
    ...
    {
        return $this->belongsToMany(ShopStore::class, 'admin_user_store', 'user_id', 'store_id');
    }
    ...
    protected static function boot()
    {
        ...
        static::deleting(function ($model) {
            if (in_array($model->id, LC_GUARD_ADMIN)) {
                return false;
            }
            $model->roles()->detach();
            ...
            if (self::allPermissions()->first(function ($permission) {
                ...
                $actions = explode(',', $permission->http_uri);
                    ...
                    if (
                        ...
                        && (
                        LC_ADMIN_PREFIX . '/config/*' == $method[1] 
                        || LC_ADMIN_PREFIX . '/config/update_info' == $method[1] 
                        || LC_ADMIN_PREFIX . '/config' == $method[1]
                        )
                    ) {
                        return true;
