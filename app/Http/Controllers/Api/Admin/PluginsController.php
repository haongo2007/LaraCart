...
class PluginsController extends RootAdminController
{
    ...
    public function index($code)
    {
        ...
        if ($action == 'config' && $pluginKey != '') {
            $namespace = bc_get_class_plugin_config($code, $pluginKey);
    ...
    protected function pluginCode($code)
    {
        ...
        $pluginsInstalled = bc_get_plugin_installed($code, $onlyActive = false);
        $plugins = bc_get_all_plugin($code);
    ...
    public function install()
    {
        $key = request('key');
        $code = request('code');
        $namespace = bc_get_class_plugin_config($code, $key);
    ...
    public function processImport() {
        ...
        $pathTmp = time();
        $linkRedirect = '';
        $pathFile = bc_file_upload($data['file'],'tmp', $pathFolder = $pathTmp)['pathFile'] ?? '';
        if ($pathFile) {
            $unzip = bc_unzip(storage_path('tmp/'.$pathFile), storage_path('tmp/'.$pathTmp));
            if ($unzip) {
                ...
                if ($checkConfig) {
                    ...
                    $arrPluginLocal = bc_get_all_plugin($configCode);
                    ...
                    try {
                        ...
                        File::deleteDirectory(storage_path('tmp/'.$pathTmp));
                        $namespace = bc_get_class_plugin_config($configCode, $configKey);
                        $response = (new $namespace)->install();
