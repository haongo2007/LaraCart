...
class TemplateOnlineController extends RootAdminController
{
    ...
    public function index()
    {
        ...
        $page = request('page') ?? 1;
        $url = config('black-cart.api_link').'/templates/?page[size]=20&page[number]='.$page;
        if(!$all_version) {
        ...
        if(!empty($dataApi['data'])) {
            ...
            return view($this->templatePathAdmin.'screen.template_online')->with(
                [
                    "title" => $title,
                    "arrTemplateLocal" => bc_get_all_template(),
