...
class PluginsOnlineController extends RootAdminController
{
    ...
    {
        $arrPluginLibrary = [];
        $bc_version = config('black-cart.core');
        ...
        $url = config('black-cart.api_link').'/plugins/'.$code.'?page[size]=20&page[number]='.$page;
        if(!$all_version) {
            $url .='&version='.$bc_version;
        }
        if($only_free) {
            $url .='&free=1';
        }
        ...
        if(!empty($dataApi['data'])) {
            foreach ($dataApi['data'] as $key => $data) {
                $arrPluginLibrary[] = [
                    ...
                    'path' => $data['path'] ?? '',
                    'file' => $data['file'] ?? '',
                    'BlackCart_version' => $data['BlackCart_version'] ?? '',
        ...
        $arrPluginLocal = bc_get_all_plugin($code);
    ...
    public function install()
    {
        ...
        try {
            ...
            $unzip = bc_unzip(storage_path('tmp/'.$pathTmp.'/'.$fileTmp), storage_path('tmp/'.$pathTmp));
            if($unzip) {
