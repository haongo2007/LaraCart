...
use BlackCart\Core\Admin\Models\AdminStoreBlockContent;
use App\Models\Front\ShopLayoutPage;
use App\Models\Front\ShopLayoutPosition;
use Validator;

class StoreBlockController extends RootAdminController
{
    ...
    public function index()
    {
        ...
        $data = [
            ...
            'subTitle'      => '',
            'icon'          => 'fa fa-indent',
            'urlDeleteItem' => bc_route_admin('admin_store_block.delete'),
        ...
        foreach ($dataTmp as $key => $row) {
            ...
            if (strpos($row['page'], '*') !== false) {
                $htmlPage .= bc_language_render('lang::layout.page_position.all');
            } else {
                $arrPage = explode(',', $row['page']);
                foreach ($arrPage as $key => $value) {
                    $htmlPage .= '+' . bc_language_render($layoutPage[$value] ?? '') . '<br>';
                }
            }
            ...
            $dataTr[] = [
                ...
                'name' => $row['name'],
                'type' => $type_name,
                'position' => htmlspecialchars(bc_language_render($this->layoutPosition[$row['position']]) ?? ''),
                'page' => $htmlPage,
                'text' => htmlspecialchars($row['text']),
                'status' => $row['status'] ? '<span class="badge badge-success">ON</span>' : '<span class="badge badge-danger">OFF</span>',
                'action' => '
                    <a href="' . bc_route_admin('admin_store_block.edit', ['id' => $row['id']]) . '"><span title="' . trans('store_block.admin.edit') . '" type="button" class="btn btn-flat btn-primary"><i class="fa fa-edit"></i></span></a>&nbsp;
        ...
        //menuRight
        $data['menuRight'][] = '
                           <a href="' . bc_route_admin('admin_store_block.create') . '" class="btn  btn-success  btn-flat" title="New" id="button_create_new">
    ...
    public function create()
    {
        ...
        $data = [
            ...
            'layout'            => [],
            'url_action'        => bc_route_admin('admin_store_block.create'),
        ];
    ...
    public function edit($id)
    {
        ...
        $data = [
            ...
            'listViewBlock' => $listViewBlock,
            'layout' => $layout,
            'url_action' => bc_route_admin('admin_store_block.edit', ['id' => $layout['id']]),
        ];
        return view($this->templatePathAdmin.'screen.store_block')
            ->with($data);
    }
    ...
    */
    ...
    {
        $arrView = [];
        foreach (glob(base_path() . "/resources/views/templates/".bc_store('template', session('adminStoreId'))."/Block/*.blade.php") as $file) {
            if (file_exists($file)) {
                $arr = explode('/', $file);
